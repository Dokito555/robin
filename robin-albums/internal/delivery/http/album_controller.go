...
func (c *AlbumController) GetAlbumListByArtistID(ctx *gin.Context) {
	req := new(model.GetAlbumRequest)
	idStr := ctx.Param("id")
	if idStr == "" {
		c.Log.Warnf("id is empty")
	...
	rsps, err := c.AlbumService.GetAlbumListByArtistID(ctx.Request.Context(), req)
	if err != nil {
		c.Log.Warnf("failed to get album list: %+v", err)
		appErr, ok := err.(*errs.AppError)
		if !ok {
			appErr = errs.ERROR_INTERNAL_SERVER_ERROR
		}
	...
	ctx.JSON(http.StatusOK, model.BaseResponse[[]model.AlbumResponse]{Message: http.StatusOK, Data: rsps})
}